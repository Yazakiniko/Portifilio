...
async function loadProjectsFromGitHub() {
    ...
    try {
        ...
        myOwnRepos.forEach(repo => {
            ...
            // --- Lógica de Tecnologias (Limpa e integrada ao CSS geral) ---
            let techsHTML = '';
            
            // Se for Java, mantém o combo de tecnologias backend pré-definido
            if (repo.language && repo.language.toLowerCase() === 'java') {
                techsHTML = `<span>Java</span><span>Oracle SQL</span><span>API Rest</span>`;
            } else {
                // Para outros projetos (CSS, JS, etc.), usa a linguagem principal ou 'Web' se não detectar nada
                techsHTML = repo.language ? `<span>${repo.language}</span>` : '<span>Web</span>';
            }
